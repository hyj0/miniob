...
RC TableMeta::init_sys_fields() {
  ...
  RC rc = field_meta.init(Trx::trx_field_name(), Trx::trx_field_type(), 0, Trx::trx_field_len(), false, UNDEFINED);
  if (rc != RC::SUCCESS) {
...
RC TableMeta::init(const char *name, int field_num, const AttrInfo attributes[]) {
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc = fields_[i + sys_fields_.size()].init(attr_info.name, attr_info.type, field_offset, attr_info.length, true, attr_info.realtype);
    if (rc != RC::SUCCESS) {
