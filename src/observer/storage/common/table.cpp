...
RC Table::insert_record(Trx *trx, Record *record) {
  ...
  if (rc != RC::SUCCESS) {
      LOG_ERROR("insert_entry_of_indexes err ret=%d:%s", rc, strrc(rc));
...
RC Table::destroy(const char *dir) {
    RC rc = sync();//刷新所有脏页
    if(rc != RC::SUCCESS) {
        LOG_ERROR("sync err ret=%d:%s", rc, strrc(rc));
        return rc;
    }

    //todo:释放buffer pool
    rc = data_buffer_pool_->close_file(this->file_id_);
    if (rc != RC::SUCCESS) {
        LOG_ERROR("close_file err ret=%d:%s", rc, strrc(rc));
        return rc;
    }
